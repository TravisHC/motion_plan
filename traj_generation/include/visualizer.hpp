...
public:
    ...
    {
        wayPointsPub = this->create_publisher<visualization_msgs::msg::Marker>(
            "/visualizer/waypoints", 10);
        trajectoryPub = this->create_publisher<visualization_msgs::msg::Marker>(
            "/visualizer/trajectory", 10);
    }

    template <int D>
    void visualize(const Trajectory<D>& traj,
                  const Eigen::Matrix3Xd& route)
    {
        ...
        wayPointsMarker.header.stamp = this->now();
        ...
        {
            std::cout<<"Clearing trajectory..."<<std::endl;
